...
def _get(url: str, params: dict | None = None, retries: int = 3) -> requests.Response | None:
    for attempt in range(retries):
        try:
            r = _scraper.get(url, params=params, headers=HEADERS, timeout=25)
            r.raise_for_status()
            return r
...
def _parse_thread(thread_url: str, site_cfg: dict) -> LinuxLynxDoc | None:
    ...
    if len(raw_logs) < 10:
        # Fallback 1: indented lines or lines matching error/log patterns
        raw_logs = "\n".join(
    ...
    if len(raw_logs) < 10:
        # Fallback 2: use the full first post text — many valid forum bug reports
        # are plain-language descriptions without code blocks.
