...
def _nvd_get(params: dict, retries: int = 3) -> dict | None:
    for attempt in range(retries):
        try:
            r = _scraper.get(NVD_API, params=params, headers=HEADERS, timeout=30)
    ...
    for keyword in keywords:
        ...
        while total < max_docs:
            params = {
                ...
                "startIndex":     offset,
                "resultsPerPage": 20,
                # noRejected is not a valid NVD 2.0 API parameter — removed
            }
            if api_key:
...
def _parse_syzbot_bug(bug_url: str) -> LinuxLynxDoc | None:
    ...
    if len(raw_logs) < 50:
        # Fallback: use the page's visible text — syzbot sometimes renders
        # crash summaries outside of <pre> blocks.
        raw_logs = soup.get_text(separator="\n")
        # Strip navigation noise by keeping only lines with log-like content
        raw_logs = "\n".join(
            ln for ln in raw_logs.splitlines()
            if re.search(r"KASAN|panic|BUG|WARNING|Oops|crash|fault|null|use-after|oob|stack", ln, re.I)
        )[:2000].strip()
