...
def _fetch_bug_detail(bug_id: int) -> LinuxLynxDoc | None:
    ...
    # --- Extract raw_logs from first comment (reporter's description)
    first_text = _strip_html(comments[0].get("text", ""))
    code_from_first = _extract_code_blocks(comments[0].get("text", first_text))
    raw_logs = code_from_first if len(code_from_first) >= 50 else first_text
    ...
    # --- Build fields
    distro    = extract_distro(all_text) if extract_distro(all_text) != "unknown" else "unknown"
...
def scrape(
    ...
    while fetched < max_bugs:
        params: dict = {
            "product":    "Platform Specific/Hardware",   # broadest kernel product
            "status":     ["RESOLVED", "VERIFIED"],
            "resolution": "FIXED",
            "limit":      PAGE_LIMIT,
            "offset":     offset,
