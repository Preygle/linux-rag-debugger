...
def _search_list(list_name: str, keyword: str, max_threads: int) -> list[str]:
    ...
    url  = f"{BASE}/{list_name}/?q={q}&x=A"   # Atom feed of search results
    html = _get_text(url)
    if not html:
        return []
...
def _is_valid_thread(messages: list[dict]) -> bool:
    ...
    # Last 3 messages should suggest resolution
    tail = "\n".join(m["raw"] for m in messages[-3:])
    if not _FIXED_RE.search(tail):
        return False
